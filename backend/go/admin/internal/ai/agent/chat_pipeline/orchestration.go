...
func BuildChatAgent(ctx context.Context, embConf config.AIEmbeddingConfig, modelConf config.ChatModelConfig, mcpURL string, metricType string) (r compose.Runnable[*UserMessage, *schema.Message], err error) {
	const (
	...
	reactAgentKeyOfLambda, err := newReactAgentLambda(ctx, embConf, modelConf, mcpURL, metricType)
	if err != nil {
		return nil, err
	}
	_ = g.AddLambdaNode(ReactAgent, reactAgentKeyOfLambda, compose.WithNodeName("ReActAgent"))
	milvusRetrieverKeyOfRetriever, err := newRetriever(ctx, embConf, metricType)
	if err != nil {
		return nil, err
	}
