...
func registerJobScheduleRouters(router fiber.Router, conf *config.Config) {
	jobScheduleHandler := logic.NewJobScheduleHandler(query.Q, appsvc.NewTemporalService(), conf.Temporal.TaskQueue)
