...
| 2026-05-21 | 本地依赖 | Postgres、Redis、Temporal 需要手动准备，没有一键启动脚本。 | 新接入代码还没有配套 Docker Compose 或本地 dev stack。 | 补 `infra/` 或脚本化本地依赖启动方式，并更新 `docs/RELIABILITY.md`。 |
...
| 2026-05-21 | 安全 | 生产级密钥注入、Cookie/CORS/CSRF、replay 防护和审计保留策略未定稿。 | 本地默认配置可运行，但不能直接代表生产安全边界。 | 补安全设计，启用或明确替代 Nonce/replay 防护，并更新 `docs/SECURITY.md`。 |
